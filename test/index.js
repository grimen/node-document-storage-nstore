...
var ENV_PREFIX = 'NSTORE';

process.env[ENV_PREFIX + '_URL_AUTHORIZED'] = process.env[ENV_PREFIX + '_URL_AUTHORIZED'] || undefined;
process.env[ENV_PREFIX + '_URL_UNAUTHORIZED'] = process.env[ENV_PREFIX + '_URL_UNAUTHORIZED'] || undefined;

console.log("\n{ENV_PREFIX}_URL_AUTHORIZED = %s\n{ENV_PREFIX}_URL_UNAUTHORIZED = %s".assign({ENV_PREFIX: ENV_PREFIX}), process.env[ENV_PREFIX + '_URL_AUTHORIZED'], process.env[ENV_PREFIX + '_URL_AUTHORIZED']);
          ...
          "(<STRING_KEY>, <VALUE>)  =>  [true]": function(done) {
            storage.set('set/new-one-foo_1-a', {foo: 'bar_1'}, function(storage_err, storage_response) {
              native.get('default-test', 'set', 'new-one-foo_1-a', function(client_err, client_response) {
                ...
                assert.deepEqual ( client_response, object({foo: 'bar_1'}) );
                done();
              });
            });
          }
            ...
            storage.set(['set/new-many-foo_1-c', 'set/new-many-foo_2-c'], [{foo: 'bar_1'}, {foo: 'bar_2'}], function(storage_err, storage_response) {
              native.get('default-test', 'set', 'new-many-foo_1-c', function(client_err_1, client_response_1) {
                native.get('default-test', 'set', 'new-many-foo_2-c', function(client_err_2, client_response_2) {
                  ...
                  assert.deepEqual ( [client_response_1, client_response_2], [object({foo: 'bar_1'}), object({foo: 'bar_2'})] );
                  done();
                });
              });
            });
