...
NStore.defaults = {
  url: process.env.NStore_URL || 'file:///tmp/{db}-{env}'.assign({db: 'default', env: (process.env.NODE_ENV || 'development')}),
  options: {
    server: {
      extension: '.db',
      mode: 0777
...
NStore.prototype.connect = function() {
  ...
  self._connect(function() {
    ...
    var db = self.options.server.db + self.options.server.extension;
...
// #key (key)
NStore.prototype.key = function(key) {
  var self = this, parts = key ? key.split('/') : [];
  return {
